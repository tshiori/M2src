...
dic = "uNeologd"
main_author = ""
#what = "WordVectorEuclid" #mainauthorの学習作品120と，他の著者のテスト作品20*9
...
model_target = model_targets[3] #ALLCorpus
#コーパス分割割合，その他
#sep = 100
sep = "T20L120"
other = "#"
...
output_file_name = m.nameFile(main_author,dic,what+"GlaphLT",target,vector,window,epoc,sep,model_target,"histmax"+str(histmax)+"-"+auth_num+"author",".png")
...
data = m.readFile(main_author,dic,what,target,vector,window,epoc,sep,model_target,other,extension,"../result/euclid/")
...
print("plot euclid distance")
fig = plt.figure()

ax = fig.add_subplot(1,1,1)
out_dir = "../result/euclid/"
...
#ファイル読み出し
#dataL = m.readFile(main_author,dic,"OneHotVectorEuclid",cate+"Learn",vector,window,epoc,sep,model_target,other,extension,"../result/euclid/")
dataL = m.readFile(main_author,dic,what,cate+"Learn",vector,window,epoc,sep,model_target,other,extension,"../result/euclid/")
...
for line in mahala_listL:
  if(line[0]==main_author):
    ...
    print("main author's learn works euclid dist is not found in...")
    print(m.nameFile(main_author,dic,what,cate+"Learn",vector,window,epoc,sep,model_target,other,extension))
    exit()
auth_dist[main_author] = np.array(list(map(float,main_distL)))
