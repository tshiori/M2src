...
#    print(line)
    if(str(line[5])=="911"):
        id = line[2]
        id_list.append(id)
        file_name = glob.glob("../data/other/"+id+"_*uNeologd")
        if(len(file_name)==1):
            if(os.path.exists(file_name[0])):
                file_list["Poetry"].append(file_name[0])
            else:
                print(id+"'s file not found")
        else:
            print(id,file_name)

print(len(file_list["Poetry"]))

#対象ファイルの読み込み
for poetry_name in file_list["Poetry"]:
    #作品の特徴量f(f_ave)の計算
...
for target in targets:
    ...
    if target=="Novel":
        ...
        plt.scatter(x, y, s=500, marker="*", linewidths=1,label="詩歌")
    else:
...
title = "全著者全作品モデルから得た宮沢賢治作品（小説／詩歌）の特徴量分布"
#plt.title(title)
plt.xlabel("第一主成分")
plt.ylabel("第二主成分")
#model_target = author+"ALLCorpus"
pca_output_filename="PCA_"+str(targets)
...
y_poetry = [1]*len(file_list["Poetry"])
...
svm = SVC(kernel=use_kernel, random_state=0, gamma=gammama_num, C=10.0)
svm.fit(X_train_std, y_train)
...
if(use_kernel==kernels[0]):
    ...
    plt.savefig('../result/svm_'+str(targets)+'_'+use_kernel+'_C'+str(Cnum)+'_gamma'+str(gammama_num)+'.png', dpi=300)
