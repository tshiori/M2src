...
for author in authors:
    ...
    print(m.nameFile(author,dic,"#",target,vector,window,epoc,sep,model_target,other,".model"))
    #m.continueOrExit()
    ...
    if data!="":
        ...
        model.save("../model/"+m.nameFile(author,dic,"#",target,vector,window,epoc,sep,model_target,other,".model"))
        print("    saved model")

        print("誤差保存:")                                                                                                                                                     
        f=open("../result/loss/"+m.nameFile(author,dic,"UseModelLoss",target,vector,window,epoc,sep,model_target,other,".txt"),"w")                                                                                                                                                 
        f.write(' '.join(list(map(str,loss))))
        f.close()

        #データの読み込みと数値リスト化
        data = m.readFile(author,dic,"UseModelLoss",target,vector,window,epoc,sep,model_target,other,".txt","../result/loss/")
        data = data.split()
        ...
        #プロットの保存
        output_file_name = m.nameFile(author,dic,"UseModelLoss",target,vector,window,epoc,sep,model_target,other,".png")
        out_dir = "../result/loss/"
